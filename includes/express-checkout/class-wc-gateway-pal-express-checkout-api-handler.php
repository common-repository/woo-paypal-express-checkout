...
class WC_Gateway_PayPal_Express_Checkout_API_Handler {
    ...
    public function getAuth() {
        try {
            if (!class_exists('Pal_Express_Checkout_For_Woo_Calculations')) {
                require_once( PAL_EXPRESS_CHECKOUT_WOO_PLUGIN_DIR . '/includes/class-woo-paypal-express-checkout-calculations.php' );
            }
            $this->gateway_calculation = new Pal_Express_Checkout_For_Woo_Calculations();
            ...
            $auth->setConfig(array('mode' => $this->mode, 'http.headers.PayPal-Partner-Attribution-Id' => 'mbjtechnolabs_SP', 'log.LogEnabled' => true, 'log.LogLevel' => ($this->mode == 'SANDBOX') ? 'DEBUG' : 'ERROR', 'log.FileName' => wc_get_log_file_path('pal_paypal_express'), 'cache.enabled' => true, 'cache.FileName' => wc_get_log_file_path('pal_express_checkout_cache')));
            return $auth;
        } catch (Exception $ex) {
            WC_Gateway_PayPal_Express_Checkout::log($ex->getMessage());
        }
    }

    public function pal_set_item_list() {
        try {
    ...
    public function set_redirect_url() {
        ...
        $this->redirecturls->setReturnUrl(esc_url_raw(add_query_arg('pal_express_checkout_action', 'return_url', WC()->api_request_url('WC_Gateway_PayPal_Express_Checkout'))));
        $this->redirecturls->setCancelUrl(esc_url_raw(add_query_arg('pal_express_checkout_action', 'cancel_url', WC()->api_request_url('WC_Gateway_PayPal_Express_Checkout'))));
    }

    public function pal_set_detail_amount() {
        try {
    ...
    public function pal_set_transaction($order) {
        try {
            ...
            $this->transaction->setAmount($this->amount)
                    ...
                    ->setNotifyUrl(apply_filters('pal_express_checkout_url', add_query_arg('pal_ipn_action', 'ipn', WC()->api_request_url('Pal_Express_Checkout_For_Woo_IPN_Handler'))))
    ...
    public function pal_cart_set_transaction() {
        try {
            $this->transaction = new Transaction();
            $this->transaction->setAmount($this->amount)
                    ->setNotifyUrl(apply_filters('pal_paypal_express_ipn_url', add_query_arg('pal_ipn_action', 'ipn', WC()->api_request_url('Pal_Express_Checkout_For_Woo_IPN_Handler'))))
                    ->setItemList($this->itemlist)
                    ->setDescription("Payment description");
        } catch (Exception $ex) {
            WC_Gateway_PayPal_Express_Checkout::log($ex->getMessage());
    ...
    public function pal_set_payer() {
        $this->payer = new Payer();
        $this->payer->setPaymentMethod("paypal");
        if (!empty($_REQUEST['is_pal_cc']) && $_REQUEST['is_pal_cc'] == 'yes') {
            $this->payer->setExternalSelectedFundingInstrumentType('CREDIT');
        }
    }

    public function pal_set_payment() {
        try {
    ...
    public function checkout_payment_url() {
        if (!empty($_POST['paymentID']) && !empty($_POST['payerID'])) {
            ...
            pal_set_session('paymentID', $paymentID);
            pal_set_session('payerID', $payerID);
    ...
    public function pal_execute_payments($order) {
        try {
            $return = $this->pal_update_payments($order);
            if ($return == false) {
                return false;
            }
    ...
    public function pal_return_url() {
        try {
            $this->getAuth();
            if (!empty($_REQUEST['paymentId']) && !empty($_REQUEST['PayerID']) && !empty($_REQUEST['token'])) {
                ...
                pal_set_session('paymentID', $paymentID);
                pal_set_session('payerID', $payerID);
                pal_set_session('token', $token);
                $payment = Payment::get($paymentID, $this->getAuth());
                $payer = $payment->getPayer();
                $this->pal_get_shipping_address($payer);
                wp_redirect(wc_get_checkout_url());
                exit();
            }
        } catch (Exception $ex) {
    ...
    public function pal_get_shipping_address($payer) {
        ...
        pal_set_session('pal_express_checkout_shipping_address', $shipping_address);
    }

    public function pal_cancel_url() {
        try {
    ...
    public function create_payment_url($bool = true) {
        ...
        try {
            if (!WC()->cart->is_empty()) {
                $this->pal_set_item_list();
                $this->set_redirect_url();
                $this->pal_set_detail_amount();
                $this->pal_cart_set_transaction();
                $this->pal_set_payer();
                $this->pal_set_payment();
    ...
    public function create_refund_request($order_id, $amount, $reason = '') {
        ...
        $this->amount->setTotal(pal_number_format($amount, $order));
        $refund = new Refund();
    ...
    public function pal_add_payment_method($card_data) {
    ...
    public function pal_return_checkout_order_received_url($order) {
        $order_id = version_compare(WC_VERSION, '3.0', '<') ? $order->id : $order->get_id();
