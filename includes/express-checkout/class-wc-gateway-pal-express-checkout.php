...
class WC_Gateway_PayPal_Express_Checkout extends WC_Payment_Gateway {
    ...
    public function __construct() {
        try {
            ...
            if (!has_action('woocommerce_api_' . strtolower('WC_Gateway_PayPal_Express_Checkout'))) {
                add_action('woocommerce_api_' . strtolower('WC_Gateway_PayPal_Express_Checkout'), array($this, 'handle_wc_api'));
            }
        } catch (Exception $ex) {
    ...
    public function init_form_fields() {
        try {
            $this->form_fields = include( 'settings-pal-express-checkout.php' );
        } catch (Exception $ex) {
    ...
    public function handle_wc_api() {
        try {
            $this->init_api();
            if (!empty($_REQUEST['pal_express_checkout_action'])) {
                $request_name = $_REQUEST['pal_express_checkout_action'];
                switch ($request_name) {
                    case 'create_payment_url': {
                            $this->pal_product_add_to_cart();
                            $this->rest_api_handler->create_payment_url();
                            break;
                        }
                    case 'execute_payment_url': {
                            $this->rest_api_handler->pal_execute_payments();
                            break;
                        }
                    case 'return_url': {
                            $this->rest_api_handler->pal_return_url();
                            break;
                        }
                    case 'cancel_url': {
                            $this->rest_api_handler->pal_cancel_url();
                            break;
                        }
    ...
    public function pal_product_add_to_cart() {
        if (!wp_verify_nonce($_POST['nonce'], '_pal_nonce_')) {
    ...
    public function process_payment($order_id) {
        ...
        if (is_pal_express_checkout_ready_to_capture()) {
            $this->init_api();
            $response = $this->rest_api_handler->pal_execute_payments($order);
    ...
    public function init_api() {
        try {
            if (!class_exists('ComposerAutoloaderInited5cff5a8f8574b72d4f6a04d4c34a6e')) {
                include_once( PAL_EXPRESS_CHECKOUT_WOO_PLUGIN_DIR . '/includes/php-library/paypal-rest/vendor/autoload.php' );
            }
            include_once( PAL_EXPRESS_CHECKOUT_WOO_PLUGIN_DIR . '/includes/express-checkout/class-wc-gateway-pal-express-checkout-api-handler.php' );
            $this->rest_api_handler = new WC_Gateway_PayPal_Express_Checkout_API_Handler();
    ...
    public static function log($message, $level = 'info') {
        if (self::$log_enabled) {
            ...
            self::$log->log($level, $message, array('source' => 'pal_express_checkout'));
        }
    }

    public function add_payment_method() {
        $this->init_api();
        $this->card_data = pal_get_posted_card($this->id);
        $response = $this->rest_api_handler->pal_add_payment_method($this->card_data);
        return $response;
    }
        ...
        if (WC_Pre_Orders_Order::order_requires_payment_tokenization($order_id)) {
            try {
                ...
                if ($this->rest_api_handler->is_request_using_save_card_data($order_id) == true) {
                    $this->rest_api_handler->pal_set_card_token($order_id);
                } else {
                    $this->rest_api_handler->pal_set_card_data($card_data);
                    if ($this->rest_api_handler->is_save_card_data() == true) {
                        $this->rest_api_handler->pal_save_card_data_in_vault();
                    }
                }
